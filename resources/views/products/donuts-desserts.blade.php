    ...
    <div class="card">
        <div class="card-body">
            <div class="text-center">
                <h2>Donuts & Desserts</h2>
            </div>
            <div class="row m-2 g-4">

                @foreach ($donuts as $dondes)
                    <div class="col-6 col-md-3">

                        <div class="card shadow h-100 border-0">

                            @if ($dondes->image)
                                <img src="{{ asset('storage/' . $dondes->image) }}" class="card-img-top"
                            ...
                            <div class="card-body">
                                <h5 class="card-title mb-1">
                                    {{ preg_replace('/^[^a-zA-Z]+/', '', $dondes->name) }}
                                </h5>
                                <p class="price mb-1">₹ {{ $dondes->price }}</p>
                                <div class="d-flex align-items-center gap-2 mt-2">
                                    @auth
                                        <a href="{{ route('orders.create', $dondes) }}"
